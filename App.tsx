        ...
        {view === 'editor' && currentNote ? (
          ...
          <div className="flex-1 flex flex-col animate-fade-in">
            <header className="px-8 pt-10 pb-6 flex items-center justify-between">
              {isSearchVisible ? (
                <div className="flex-1 flex items-center bg-white dark:bg-gray-800 border border-indigo-100 dark:border-gray-700 rounded-2xl h-14 shadow-sm mr-4 px-4 animate-in slide-in-from-left duration-300">
                  <Search size={20} className="text-indigo-500 mr-3" />
                  <input 
                    ref={searchInputRef}
                    type="text" 
                    placeholder="SEARCH YOUR INSIGHTS..." 
                    className="flex-1 bg-transparent label-caps tracking-widest outline-none text-xs"
                    value={searchQuery}
                    onChange={(e) => setSearchQuery(e.target.value)}
                  />
                  <button onClick={() => { setIsSearchVisible(false); setSearchQuery(''); }} className="p-1 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-full transition-colors">
              ...
              <div className="flex items-center gap-2">
                {!isSearchVisible && (
                  <button 
                    ...
                    className={`w-12 h-12 rounded-xl border flex items-center justify-center shadow-sm transition-all active:scale-90 ${isDarkMode ? 'bg-gray-800 border-gray-700 text-indigo-400' : 'bg-white border-gray-100 text-indigo-600'}`}
                  >
                    <Search size={22} />
                  </button>
                )}
            ...
            <main className="flex-1 overflow-y-auto px-8 pt-6 pb-32 custom-scrollbar">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-6xl mx-auto">
                {filteredNotes.map(note => (
                  <NoteCard 
                    key={note.id} 
                    note={note} 
                    onClick={() => { setCurrentNote(note); setView('editor'); setHistory([note.content]); setHistoryIndex(0); setIsSearchVisible(false); }}
              ...
              {filteredNotes.length === 0 && (
                <div className="h-64 flex flex-col items-center justify-center opacity-20 text-center">
                  ...
                  <span className="label-caps">{searchQuery ? "No matches found" : "Your digital legacy starts here"}</span>
                </div>
              )}
            </main>
            ...
            <nav className={`fixed bottom-0 left-0 right-0 border-t flex flex-col items-center z-50 ${isDarkMode ? 'bg-[#0f111a]/95 border-gray-800' : 'bg-white/95 border-gray-50'} backdrop-blur-xl`}>
              <div className="w-full flex items-center justify-around px-4 pt-4 pb-8 max-w-lg mx-auto">
                <NavItem icon={<Home />} active={view === 'home'} onClick={() => { setView('home'); setIsSearchVisible(false); }} label="Home" isDark={isDarkMode} />
                <NavItem icon={<Star />} active={view === 'starred'} onClick={() => { setView('starred'); setIsSearchVisible(false); }} label="Starred" isDark={isDarkMode} />
                ...
                <NavItem icon={<Settings />} active={view === 'settings'} onClick={() => { setView('settings'); setIsSearchVisible(false); }} label="Settings" isDark={isDarkMode} />
              </div>
