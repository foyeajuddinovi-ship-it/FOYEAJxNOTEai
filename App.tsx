  ...
  return (
    <div onClick={onClick} className={`flex items-center transition-all ${onClick ? 'cursor-pointer hover:opacity-80 active:scale-95' : ''} ${size === 'lg' ? 'gap-3' : 'gap-2'}`}>
      ...
      <div className="flex flex-col">
        ...
        <span className={`font-black uppercase tracking-[0.2em] leading-none mt-1 ${subTextColor} ${size === 'sm' ? 'text-[7px]' : size === 'md' ? 'text-[10px]' : 'text-[12px]'}`}>Note</span>
      </div>
    </div>
  );
};
...
const App: React.FC = () => {
  ...
  const onGenerateImage = async (prompt: string) => {
    ...
    } catch (e) { console.error(e); alert("Failed to generate image."); } finally { setIsGeneratingImage(false); }
  };
  ...
  return (
    <div className={`h-screen w-full flex overflow-hidden font-['Inter'] antialiased transition-colors duration-300 ${isDarkMode ? 'dark bg-[#0f111a]' : 'bg-[#F9FAFF]'}`}>
      <style>{`
      ...
      <div className="flex-1 h-full overflow-hidden relative">
        {view === 'editor' && currentNote ? (
          <div className={`flex flex-col h-full transition-colors duration-300 ${isDarkMode ? 'bg-[#0f111a]' : 'bg-white'} font-['Inter'] relative`}>
            {showSummaryPopUp && (
                ...
                <div className={`${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-indigo-100'} rounded-[32px] border shadow-[0_40px_80px_rgba(72,86,169,0.15)] overflow-hidden`}>
                  <div className={`px-8 py-6 ${isDarkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-indigo-50/50 border-indigo-50'} flex items-center justify-between border-b`}>
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 bg-[#4856a9] rounded-2xl flex items-center justify-center text-white shadow-lg"><Sparkles size={20} /></div>
                      <div>
                    ...
                    <div className="flex items-center gap-3">
                      <button onClick={handleApplySummary} className="flex items-center gap-2 bg-[#4856a9] text-white px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg active:scale-95 transition-all hover:bg-[#3b4791]"><Copy size={12} /> Apply</button>
                      <button onClick={() => setShowSummaryPopUp(false)} className={`p-2.5 rounded-xl transition-all active:scale-90 ${isDarkMode ? 'bg-gray-700 border-gray-600 text-gray-400' : 'bg-white border border-indigo-100 text-indigo-400 hover:text-red-500 hover:border-red-100'}`}><X size={18} strokeWidth={3} /></button>
                    </div>
                  </div>
                  <div className="px-8 py-7"><p className={`${isDarkMode ? 'text-gray-200' : 'text-gray-700'} font-medium leading-relaxed italic text-sm`}>"{summaryContent}"</p></div>
                </div>
              </div>
            )}
            <div className={`px-6 py-8 transition-colors duration-300 ${isDarkMode ? 'bg-[#0f111a] border-gray-800' : 'bg-white border-gray-100/50'} z-20 border-b`}>
            ...
            <div className="px-6 py-2 relative overflow-visible">
              ...
              {showDictateDropdown && <div className="absolute top-[60px] left-6 z-[100]"><LanguageDropdown title="Speak to Text" subtitle="What language will you speak in?" position="bottom" onClose={() => setShowDictateDropdown(false)} onSelect={(lang) => { startRecording(lang); setShowDictateDropdown(false); }} activeLangCode={dictateLang.code} /></div>}
              ...
              <div className="overflow-visible pb-12 -mb-12">
                <div className={`${isDarkMode ? 'bg-[#313b7a]' : 'bg-[#4856a9]'} rounded-2xl p-1.5 flex items-center justify-between shadow-[0_15px_40px_rgba(72,86,169,0.25)] transition-colors duration-300`}>
                  <div className="flex items-center gap-1.5">
                    <ToolbarIcon onClick={handleLoud} active={isLouding} loading={isLouding} tooltipTitle="Vocalizer"><Volume2 size={20} /></ToolbarIcon>
                    <ToolbarIcon onClick={handleDictateToggle} active={isTranscribing} badge={isTranscribing ? dictateLang.icon : undefined} className={isTranscribing ? "bg-red-500 hover:bg-red-600 animate-pulse" : ""} tooltipTitle="Dictation">{isTranscribing ? <MicOff size={20} /> : <AudioLines size={20} />}</ToolbarIcon>
                    <ToolbarIcon onClick={() => { setShowTranslateDropdown(!showTranslateDropdown); setShowStyling(false); setShowDictateDropdown(false); }} active={showTranslateDropdown} loading={isTranslating} badge={selectedLang.icon} tooltipTitle="Translate"><Languages size={20} /></ToolbarIcon>
                    <ToolbarIcon onClick={handleSummarize} loading={isSummarizing} active={isSummarizing} tooltipTitle="Summary"><User size={20} /></ToolbarIcon>
                    <ToolbarIcon onClick={handleGallery} tooltipTitle="Gallery"><ImageIcon size={20} /></ToolbarIcon>
                    <ToolbarIcon onClick={() => setShowAIArtModal(true)} loading={isGeneratingImage} tooltipTitle="AI Art"><ImagePlus size={20} /></ToolbarIcon>
                    <div className="w-[1.5px] h-7 bg-white/20 mx-1.5"></div>
                    <ToolbarIcon onClick={() => { if (historyIndex > 0) { const p = history[historyIndex - 1]; setHistoryIndex(historyIndex - 1); if (editorRef.current) editorRef.current.innerHTML = p; } }} active={historyIndex > 0} tooltipTitle="Undo"><RotateCcw size={20} /></ToolbarIcon>
                    <ToolbarIcon onClick={() => { if (historyIndex < history.length - 1) { const n = history[historyIndex + 1]; setHistoryIndex(historyIndex + 1); if (editorRef.current) editorRef.current.innerHTML = n; } }} active={historyIndex < history.length - 1} tooltipTitle="Redo"><RotateCw size={20} /></ToolbarIcon>
                  ...
                  <div className="flex items-center gap-1.5 ml-8 pr-2">
                    <ToolbarIcon onClick={handleDownload} tooltipTitle="Download Note"><Download size={20} /></ToolbarIcon>
                    <ToolbarIcon onClick={() => { if (confirm("PERMANENTLY CLEAR ALL NOTE CONTENT?")) { if (editorRef.current) editorRef.current.innerHTML = ''; setCurrentNote({...currentNote, content: ''}); addToHistory(''); } }} tooltipTitle="Clear All Content"><Eraser size={20} /></ToolbarIcon>
                    <ToolbarIcon onClick={async () => { try { await navigator.share({ title: currentNote.title, text: getPlainText(editorRef.current?.innerHTML || '') }); } catch (e) {} }} tooltipTitle="Share"><Share2 size={20} /></ToolbarIcon>
                  </div>
                </div>
              </div>
            </div>
                  ...
                  <div className="flex gap-4 mb-6 overflow-x-auto no-scrollbar pb-2">
                    {currentNote.images.map((img, idx) => (
                      <div key={idx} className="relative group flex-shrink-0">
                        ...
                        <button onClick={() => { const newImages = currentNote.images.filter((_, i) => i !== idx); const updatedNote = {...currentNote, images: newImages}; setCurrentNote(updatedNote); setNotes(prev => prev.map(n => n.id === updatedNote.id ? updatedNote : n)); }} className="absolute top-2 right-2 p-1.5 bg-black/50 text-white rounded-full opacity-0 group-hover:opacity-100 transition-opacity"><X size={14} /></button>
                      </div>
                    ))}
                  </div>
                )}
                <div ref={editorRef} contentEditable onPaste={handlePaste} data-placeholder="BEGIN YOUR CREATIVE JOURNEY..." className={`flex-1 w-full rich-editor leading-relaxed text-xl transition-colors duration-300 ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`} onInput={(e) => { const h = e.currentTarget.innerHTML; setCurrentNote({...currentNote, content: h}); addToHistory(h); }} dangerouslySetInnerHTML={{ __html: currentNote.content }} />
              </main>
            </div>
            <div className="absolute bottom-10 right-10 z-[5000]">
          ...
          <div className={`flex flex-col h-full transition-colors duration-300 ${isDarkMode ? 'bg-[#0f111a]' : 'bg-[#F9FAFF]'} overflow-hidden relative`}>
            <header className="px-6 pt-10 pb-4 flex items-center justify-between">
              <div className="flex items-center gap-4">
                <Logo variant={isDarkMode ? 'white' : 'default'} size="md" />
              </div>
              <div className="relative">
                <button onClick={() => setShowThemeMenu(!showThemeMenu)} className={`w-12 h-12 rounded-full border flex items-center justify-center shadow-lg active:scale-95 transition-all ${isDarkMode ? 'bg-gray-800 border-gray-700 text-indigo-300' : 'bg-white border-gray-100 text-[#4856a9]'}`}>{isDarkMode ? <Moon size={24} /> : <Sun size={24} />}</button>
                {showThemeMenu && (
            ...
            <div className="px-6 py-6">
              <div className="relative w-full group">
                <div className={`absolute inset-0 blur-xl opacity-20 ${isDarkMode ? 'bg-indigo-500' : 'bg-[#4856a9]'} transition-all group-focus-within:opacity-40`}></div>
                <div className={`relative flex items-center border rounded-full overflow-hidden transition-all h-16 ${isDarkMode ? 'bg-gray-800/50 border-gray-700 focus-within:border-indigo-500' : 'bg-white border-gray-100 focus-within:border-[#4856a9]'}`}>
                  <Search className={`absolute left-6 transition-colors ${isDarkMode ? 'text-gray-600 group-focus-within:text-indigo-400' : 'text-[#4856a9]/30 group-focus-within:text-[#4856a9]'}`} size={20} strokeWidth={3} />
                  <input type="text" placeholder="SEARCH YOUR NOTES" className={`w-full bg-transparent py-4 pl-16 pr-10 text-sm font-bold tracking-tight focus:outline-none placeholder:uppercase ${isDarkMode ? 'text-gray-100 placeholder:text-gray-600' : 'text-gray-900 placeholder:text-gray-200'}`} value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} />
                </div>
              </div>
            </div>
            <main className="flex-1 overflow-y-auto px-6 pb-40 no-scrollbar">
              <div className="max-w-4xl mx-auto">
                <div className="grid grid-cols-1 gap-6">
                  {notes.filter(n => n.title.toLowerCase().includes(searchQuery.toLowerCase()) || n.content.toLowerCase().includes(searchQuery.toLowerCase())).filter(n => view === 'starred' ? n.isStarred : true).sort((a, b) => (a.isPinned === b.isPinned ? 0 : a.isPinned ? -1 : 1)).map(note => (
                      <NoteCard 
                        key={note.id} 
                        note={note} 
                        ...
                        onDelete={(e) => { e.stopPropagation(); if (confirm("ARE YOU SURE? THIS WILL PERMANENTLY DELETE THE PAYMENT SYSTEM.")) setNotes(prev => prev.filter(n => n.id !== note.id)); }} 
                        isDark={isDarkMode} 
                      />
                  ))}
                </div>
                {notes.length === 0 && (
                  <div className="flex flex-col items-center justify-center py-24 opacity-30 text-center">
                    <div className={`w-20 h-20 rounded-full flex items-center justify-center mb-6 ${isDarkMode ? 'bg-gray-800' : 'bg-gray-100'}`}>
                      <Edit3 size={40} className={isDarkMode ? 'text-gray-600' : 'text-gray-300'} />
                    </div>
            ...
            <div className={`fixed bottom-0 left-0 right-0 border-t flex flex-col items-center z-[100] transition-colors duration-300 ${isDarkMode ? 'bg-[#0f111a] border-gray-800' : 'bg-white border-gray-50'}`}>
              <div className="w-full flex items-center justify-around px-4 pt-4 pb-12 shadow-[0_-15px_40px_rgba(0,0,0,0.02)]">
                <NavItem icon={<Home size={28} strokeWidth={2.5} />} active={view === 'home'} onClick={() => setView('home')} tooltipTitle="Home" isDark={isDarkMode} />
                <NavItem icon={<Star size={28} strokeWidth={2.5} />} active={view === 'starred'} onClick={() => setView('starred')} tooltipTitle="Starred" isDark={isDarkMode} />
                <div className="relative">
                  <button onClick={handleCreateNote} onMouseEnter={() => setShowAddTooltip(true)} onMouseLeave={() => setShowAddTooltip(false)} className={`w-16 h-16 rounded-3xl flex items-center justify-center shadow-2xl -mt-12 border-[6px] active:scale-90 transition-all ${isDarkMode ? 'bg-[#4856a9] text-white border-gray-900 shadow-indigo-900/50' : 'bg-[#4856a9] text-white border-[#F9FAFF] shadow-indigo-200'}`}>
                    <div className="bg-white/20 p-2 rounded-xl"><Plus size={32} strokeWidth={4} /></div>
                  </button>
                  <Tooltip text="Create New" visible={showAddTooltip} position="top" />
                </div>
                <NavItem icon={<Bot size={28} strokeWidth={2.5} className={isDarkMode ? 'text-indigo-400' : 'text-[#4856a9]'} />} active={view === 'ai'} onClick={() => setShowAIChat(true)} tooltipTitle="AI Intel" isDark={isDarkMode} />
                <NavItem icon={<Settings size={28} strokeWidth={2.5} />} active={view === 'settings'} onClick={() => setView('settings')} tooltipTitle="Settings" isDark={isDarkMode} />
              </div>
              <div className={`w-full py-2.5 flex items-center justify-center gap-2 border-t ${isDarkMode ? 'border-gray-800 bg-gray-950/20' : 'border-gray-100 bg-gray-50/50'}`}>
                ...
                <span className={`text-[8px] font-black uppercase tracking-[0.25em] ${isDarkMode ? 'text-gray-500' : 'text-gray-400'}`}>{isSyncing ? 'Syncing foyeajX Cloud Assets...' : 'Secure Cloud Encrypted'}</span>
      ...
      {showAIChat && (
        <div className="fixed inset-0 z-[6000] bg-black/40 backdrop-blur-md flex items-center justify-end">
          <div className={`w-full max-w-md h-full sm:h-[90vh] sm:rounded-[48px] sm:mr-10 shadow-2xl overflow-hidden animate-in slide-in-from-right duration-500 ease-out border transition-colors ${isDarkMode ? 'bg-[#0f111a] border-gray-800' : 'bg-white border-white/50'}`}>
